...
export default function PurchaseOrdersPage() {
    ...
    const handleSubmitOrder = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const items = orderLines.map(l => ({
                ...
                temp_product_data: l.is_new ? { name: l.name, price: l.price || 0, category: l.category || 'Général' } : null
            }))

            await supabase.from('purchase_order_items').insert(items)

            showToast(editingOrder ? "Bon d'achat mis à jour !" : "Bon de commande créé !", "success")
            closeModal()
            fetchOrders()
        } catch (err) {
    ...
    const handleEditOrder = async (order: any) => {
        if (order.status === 'received') return showToast("Impossible de modifier un bon déjà reçu", "info")
        ...
        if (items) {
            setOrderLines(items.map(i => ({
                product_id: i.product_id,
                name: i.products?.name || i.temp_product_data?.name || "Produit inconnu",
                quantity: i.quantity,
                cost_price: i.cost_price,
                is_new: !i.product_id,
                price: i.temp_product_data?.price || 0,
                category: i.temp_product_data?.category || 'Général'
            })))
        }
        setIsModalOpen(true)
    }
                ...
                {loading && orders.length === 0 ? (
                    ...
                    <div key={order.id} className="glass-panel p-6 rounded-[32px] border-white/5 hover:border-shop/30 transition-all group relative overflow-hidden flex flex-col justify-between">
                        <div className="flex items-start justify-between mb-6">
                            ...
                            <span className={`px-3 py-1.5 rounded-xl text-[8px] font-black uppercase border shadow-lg ${order.status === 'received' ? 'bg-green-500/10 border-green-500/20 text-green-400' : 'bg-orange-500/10 border-orange-500/20 text-orange-400 animate-pulse'
                                }`}>
                        ...
                        {order.status === 'pending' ? (
                            <div className="flex gap-2">
                                <button
                                    ...
                                    className="flex-1 py-4 bg-white text-black hover:bg-shop hover:text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] flex items-center justify-center space-x-2 shadow-2xl transition-all active:scale-95"
                                >
                            ...
                            <div className="flex gap-2">
                                <div className="flex-1 flex items-center justify-center py-4 bg-green-500/5 rounded-2xl border border-green-500/10 text-[9px] font-black uppercase text-green-400/60 tracking-widest">
                                    Archivé
                                </div>
                                <button
                                    onClick={() => handleDeleteOrder(order)}
                                    className="p-4 bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white rounded-2xl transition-all"
                                    title="Supprimer et annuler le stock"
                                >
                                    <Trash2 className="w-4 h-4" />
                                </button>
                            </div>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 backdrop-blur-3xl bg-black/60">
                    <div className="relative glass-card w-full max-w-3xl p-8 sm:p-12 rounded-[48px] shadow-[0_0_100px_rgba(var(--shop-primary),0.1)] border-white/10 animate-in zoom-in-95 duration-300 flex flex-col max-h-[94vh]">
                        <button onClick={closeModal} className="absolute top-10 right-10 p-3 bg-white/5 hover:bg-white/10 rounded-full transition-all"><X className="w-6 h-6 text-white" /></button>
                        ...
                        <div className="flex items-center space-x-6 mb-12">
                            <div className="w-20 h-20 bg-shop/20 text-shop rounded-[32px] flex items-center justify-center border border-shop/20 shadow-2xl">
                                {editingOrder ? <FileText className="w-10 h-10" /> : <PlusCircle className="w-10 h-10" />}
                            </div>
                            <div>
                                <h2 className="text-4xl font-black uppercase tracking-tighter text-white italic">
                                    {editingOrder ? 'Modifier' : 'Créer'} un <span className="text-shop">Bon d'Achat.</span>
                                </h2>
                                <p className="text-[11px] text-muted-foreground font-bold tracking-[0.3em] uppercase mt-1.5">
                                    {editingOrder ? `Modification de l'achat #${editingOrder.id.slice(0, 8)}` : "Entrée de marchandises multiproduits"}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="pt-8 border-t border-white/10 mt-auto flex flex-col sm:flex-row justify-between items-center gap-8">
                            ...
                            <button onClick={handleSubmitOrder} disabled={creating || orderLines.length === 0} className="w-full sm:w-auto px-16 py-6 bg-white text-black hover:bg-shop hover:text-white font-black uppercase tracking-[0.3em] rounded-[32px] hover:scale-105 active:scale-95 transition-all shadow-2xl text-[11px] disabled:opacity-20 disabled:grayscale">
                                {creating ? <Loader2 className="animate-spin mx-auto" /> : (editingOrder ? "Mettre à jour" : "Émettre le Bon d'Achat")}
                            </button>
                        </div>
                    </div>
                </div>
