...
export default async function Home(props: { 
  ...
  const [{ products: filteredProducts, totalCount }, siteSettings] = await Promise.all([
      getProducts({ 
          page: currentPage, 
          shopId: shopFilter, 
          cat: catFilter, 
  ...
  // For the categories and brands lists in the sidebar
  const categories = Array.from(new Set(filteredProducts.map((p: any) => p.category).filter(Boolean))).sort() as string[];
